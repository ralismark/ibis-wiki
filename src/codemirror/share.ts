...
function shareFor<T>(
	type: StateEffectType<T>,
): (spec: Omit<EffectSharingSpec<T>, "type">) => EffectSharingSpec<T> {
	return (spec: any) => {
		spec.type = type
		return spec
	}
...
export function effectsFromJSON(json: sharedEffect[], state: EditorState): StateEffect<any>[] {
