...
export class FileWidget extends IWidget {
	...
	show(ctl: WidgetControl): WidgetContent {
		...
		const subpages = useMemo(() => {
			if (!facade) return []
			return Array.from(facade.listing.getSnapshot().values()).filter((p) => p.startsWith(this.path + ":"))
		}, [facade])

		const superpages = useMemo(() => {
			if (!facade) return []
			return Array.from(facade.listing.getSnapshot().values()).filter((p) => this.path.startsWith(p + ":"))
		}, [facade])
				...
				<ul>
					{bottomMeta.map(
						([key, items]) =>
							items.length > 0 && (
								<li key={key}>
									{key} ({items.length}): {items.map((p) => (
										<button key={p} onClick={() => ctl.open(new FileWidget(p))}>
											{p}
										</button>
									))}
								</li>
