			...
			async (cleanup: Promise<void>) => {
				...
				await facade.fts.all((row) => {
					...
					for (const linked of row.refs) {
						if (paths.has(linked)) links.push({ source: row.path, target: linked })
					}
					...
					while (path.includes(":")) {
						path = path.replace(/:[^:]*$/, "")
						if (paths.has(path)) links.push({ source: row.path, target: path })
					}
				})
				...
				const simulation = d3
					...
					.force(
						"charge",
						d3.forceManyBody().strength((d) => (d as Node).id.length * -10),
					)
					.force("x", d3.forceX())
					.force("y", d3.forceY())
					.on("tick", ticked)
				...
				label.call(
					d3.drag().on("start", dragstarted).on("drag", dragged).on("end", dragended) as any,
				)
