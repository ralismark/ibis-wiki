...
export class FullTextSearch {
	...
	constructor(fetcher: (path: string) => Promise<Snapshot>) {
		this.db = new Promise((resolve, reject) => {
			...
			r.onupgradeneeded = function() {
				// function to rebind this so we get the db
	...
	private marchCursors(
		cursorReqs: IDBRequest<IDBCursorWithValue | null>[],
		each: (row: FtsRow) => void,
	): Promise<void> {
	...
	async all(each: (row: FtsRow) => void): Promise<void> {
		await this.tr("fts", "readonly", async (tr) => {
			await this.marchCursors([tr.objectStore("fts").openCursor()], each)
		})
	}
}
